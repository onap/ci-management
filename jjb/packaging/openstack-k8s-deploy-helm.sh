...
helm3.7 install sdnc onap --dry-run
echo "INFO: ODL Helm Charts install"
helm3.7 install sdnc onap

POD_NAME=$(kubectl get pods --namespace default -l "app.kubernetes.io/name=onap,app.kubernetes.io/instance=sdnc" -o jsonpath="{.items[0].metadata.name}")
