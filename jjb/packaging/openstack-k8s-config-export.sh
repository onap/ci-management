...
for try in $(seq $CLUSTER_NODE_RETRIES); do
    sleep 30
    mapfile -t OS_NODES < <(openstack --os-cloud "$os_cloud" server list -f value -c "Name" | grep -E "kube.*")
...
for node in "${OS_NODES[@]}"; do
    echo "node: $node"
    if [[ "$node" =~ kube.*default-worker.* ]]; then
        K8S_NODE="${node}"
        openstack --os-cloud vex server add security group "$K8S_NODE" "default"
...
if [[ -n "${MAIN_NODE}" ]]; then
    ...
    NETWORK_ID="bd089be9-e846-4afe-905b-8bb6c7c90faf" #"$(openstack --os-cloud vex network show -f value -c id "$SILO-$VM_NAME")"
    openstack --os-cloud vex server add network "$SERVER_ID" "$NETWORK_ID"
    openstack --os-cloud vex server add security group "$MAIN_NODE" "default"
    ...
    if [ "$FACTER_OS" == "centos" ]; then
        ...
        case "$FACTER_OSVER" in
            20.04)
                ...
                sudo sed -i "/dhcp4: true/d" "$ENS7_SCRIPT"
                cat "$ENS7_SCRIPT"
                sudo netplan apply
            ;;
            *)
    ...
    MAIN_IP=$(openstack --os-cloud "${os_cloud}" server list -f value -c Networks -c Name --name $MAIN_NODE | awk -F"'" '{print $4}')

    # Get internal IP of worker node in the cluster
    mapfile -t NODE_IPS < <(openstack --os-cloud "${os_cloud}" server list -f value -c Networks -c Name --name "kube.*default-worker.*" | awk -F"'" '{print $4}')
